...
public class GraphLoader 
{
	 ...
	 *   described.
	 * @param theMap The graph to load the map into.  The graph is
	 ...
	 */
	public static void loadRoadMap(String filename, MapGraph theMap)
	{
		loadRoadMap(filename, theMap, null, null);
	}
			...
			Set<GeographicPoint> intersectionsToLoad)
	{
		...
		addEdgesAndSegments(nodes, pointMap, theMap, segments);
	}
			...
			HashMap<GeographicPoint,HashSet<RoadSegment>> segments)
	{
		...
		for (GeographicPoint pt : nodes) {
			...
			for (RoadLineInfo info : outgoing) {
				...
				theMap.addEdge(pt, end, info.roadName, info.roadType, length);
